...
class ChessMixin:
    ...
    def print_chessboard(cls,chessboard=None):
        ...
        for i in chessboard.values():
            if i=='':
                print("| ",end="")
            else:print("|"+cls.gui_text[i[:2]], end='')
            if index==7:index=0;print("|\n+-+-+-+-+-+-+-+-+");
            else:index += 1;
        ...
        if castle[man[0]]['km']==False and (man[1]=='E' or man[1]=='K'):
            if man[1]=='K':
                if int(from_[1])-int(to_[1])==2:
                    ...
                    chessboard[castle[man[0]]['lep']]=''
                    chessboard[castle[man[0]]['lboxes'][0]]=castle[man[0]]['leid']
    ...
    def statistics(self):
        ...
        #chessman positon on board score
    ...
    # black_pos = ['33','34','43','44']
    # white_pos = ['43','44', '33','34']
    ...
    def chessmans_can_move_to_box(block_possible_moves):
        """ this method return dictionart of number of chessmans can moves to particuler position of one side(black or white)
        like box_backups-->{'45':['BP62','BP65','BC72']}""" 
        box_backups = {}
        for chessman,moves in block_possible_moves.items():
            if len(moves)>0:
                for m in moves:
                    if m in box_backups.keys():box_backups[m].append(chessman); 
                    else: box_backups[m] = [chessman];
        return box_backups                
        ...
        for k,v in  my_chessmans_moves.items():
            if len(v)>0:
                for _ in opp_chessmans_positions.values():
                    if _ in v:
                        if k in x.keys():x[k].append(_);
                        else:x.setdefault(k,[_]);
        return x

    @staticmethod
    def check_threats(chessmans_positions,opponent_all_moves):
            ...
            if pos in opponent_all_moves:
                if man[1]=='S':
                    if abs(int(man[2])-int(pos[0])) == 6:
                        threats[man] = self.chessman_points[man[1]]*6
                    elif abs(int(man[2])-int(pos[0])) == 5:
                        threats[man] = self.chessman_points[man[1]]*2
                    elif abs(int(man[2])-int(pos[0])) == 4:
                        threats[man] = self.chessman_points[man[1]]*1.5
                    else:
                        threats[man] = self.chessman_points[man[1]]
                else:
                    if man[1]!='K':
                        threats[man] = self.chessman_points[man[1]]
        return threats
